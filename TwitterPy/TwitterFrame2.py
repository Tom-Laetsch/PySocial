...
def standardize_twitframe( twitframe,
    ...
    if 'text' in twitframe.columns.values:
        if 'tokenized_text' not in twitframe.columns.values:
            ...
            twitframe['tokenized_text'] = tokenize_text( twitframe, verbose = False )
        #extract emoji present
        if 'emojis' not in twitframe.columns.values:
            if verbose: print("Extracting emojis...")
            twitframe['emojis'] = extract_emojis( twitframe, verbose = False )
    #create hashtags and mentions columns
    if 'entities' in twitframe.columns.values:
        if 'hashtags' not in twitframe.columns.values:
            if verbose: print("Extracting hashtags...")
            twitframe['hashtags'] = extract_hashtags( twitframe, verbose = False )
